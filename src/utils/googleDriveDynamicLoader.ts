...
const API_BASE_URL = process.env.NODE_ENV === 'production' 
  ? process.env.VITE_API_BASE_URL || "https://your-backend-url.railway.app/api"
 ...
 * This is completely dynamic - no hardcoded values!
 */
const discoverGoogleDriveStructure = async (): Promise<GoogleDriveFolder[]> => {
  try {
    console.log('🔄 Dynamically discovering folder structure from Google Drive API...');
    ...
    folders.forEach(folder => {
      console.log(`📁 Folder: "${folder.name}" (${folder.files.length} files)`);
    ...
    throw new Error("Failed to discover Google Drive folder structure. Please ensure the backend server is running and Google Drive API is configured.");
  }
};

/**
 * Check if a folder has all required files (JSON, CSV, PNG)
 * This dynamically discovers files - no hardcoded file names!
 */
export const checkGoogleDriveFolderContents = async (folderName: string): Promise<{
  ...
  try {
    ...
    // Dynamically find files by extension and MIME type - no hardcoded names!
 ...
 * This is completely dynamic - discovers all folders automatically!
 */
export const getGoogleDriveFolders = async (): Promise<string[]> => {
  try {
    console.log('🔄 Dynamically discovering all available folders...');
...
export const clearGoogleDriveCache = () => {
  ...
  console.log('🧹 Google Drive cache cleared');
};
